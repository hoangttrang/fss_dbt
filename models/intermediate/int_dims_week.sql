...
week_boundaries AS (
    ...
    SELECT
        date_day,
        -- Shift back to Monday (ISO: dow=1 means Monday, dow=0 means Sunday)
        date_day - ((EXTRACT(DOW FROM date_day)::INT + 6) % 7) AS week_start_date,
        -- Always week_start_date + 6
        date_day - ((EXTRACT(DOW FROM date_day)::INT + 6) % 7) + 6 AS week_end_date
    FROM date_spine
)
