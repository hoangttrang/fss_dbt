WITH date_map AS ( 
    SELECT DISTINCT * FROM {{ ref('int_dims_week') }}
)
...
WHERE budget_month1.total_amnt IS NOT NULL AND budget_month2.total_amnt IS NOT NULL) 
...
select * from calculate_weekly_budget